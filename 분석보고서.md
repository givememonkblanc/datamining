...
왜도 2.65의 Right-skewed 분포는 **대부분의 고객은 비교적 짧은 주기로 방문하지만, 일부는 매우 긴 간격**을 둔다는 점을 보여준다. Q1(49일) 이내에 전체의 46.3%가 집중되어 있어, 절반 가까운 방문이 예측하기 쉬운 단기 패턴(정기점검/오일교환)임을 암시하는 반면, Q3(166일) 이상의 긴 꼬리 구간(25%)은 예측이 어려운 비정기 방문이다.
...
| Cluster | 명칭 | 차량수 | 평균주행 | 평균간격 | 방문빈도 | 특성 |
|:-------:|------|:-----:|:--------:|:--------:|:--------:|------|
| 1 | **중간주행군** | 56,894대 | 76,570km | 115.0일 | 2.9회/년 | **가장 일반적인 패턴**, 중간 수준 |
| 0 | **저주행군** | 42,014대 | 51,231km | 171.3일 | 1.9회/년 | 상대적으로 주행량이 낮고 방문 간격이 김 |
| 2 | **고주행군** | 16,016대 | 157,149km | **57.3일** | **6.7회/년** | 주행량이 높고 방문이 매우 잦음 |
| 3 | **초저주행군** | 5,049대 | 107,965km | 172.2일 | 2.0회/년 | 일평균 주행이 매우 낮고 방문도 드문 편 |
...
### 3.1 고객 세분화를 위한 클러스터링 — Unsupervised Learning (LN00)

**비지도학습(Unsupervised Learning)** 접근법으로, `log_drivingKm`와 `log_DayAvgDrivingKm` 두 변수(표준화 후)에 대해 **K-Means 클러스터링(k=4)** 을 수행하였다. 클러스터링 전에 `DayAvgDrivingKm > 0`, `drivingKm > 100` 조건을 적용하고 상위 극단값을 절단한 뒤 표준화하였다. 이는 사전 정보(레이블) 없이 데이터 내 **잠재된 그룹 구조(Latent Group Structure)** 를 발견하는 과정이다.

중요한 점은, 본 군집화는 **방문간격 변수로 직접 세분화한 것이 아니라 주행량 기반 세그먼트화**라는 점이다. 따라서 군집의 1차 의미는 "방문 패턴이 비슷한 차량"이 아니라 **주행거리와 일평균 주행거리가 비슷한 차량군**이다. 평균 방문간격/방문빈도는 그 군집을 설명하기 위해 **후속적으로 덧붙인 해석 변수**이다. 또한 본 코드에서는 `k=4`를 고정해 사용하였으므로, 이는 **최적 k 탐색 결과**라기보다 **4개 세그먼트 요약**에 해당한다. 군집명은 실제 용도 추정(예: 영업용/업무용, 장기 미사용)이 아니라 **저주행군·중간주행군·고주행군·초저주행군**처럼 주행 강도에 기반한 설명적 라벨만 사용한다.
...
|:-------:|:---:|:----------:|:------------:|:-------:|:----------:|:------|
| **0** | 42,014건 | 46,229km | 43km/일 | 2.0회/년 | 171.3일 | 저주행군 |
| **1** | 56,894건 | 122,870km | 100km/일 | 3.6회/년 | 115.0일 | 중간주행군 (**최대 비중**) |
| **2** | 16,016건 | 799,920km | 271km/일 | 6.5회/년 | 57.3일 | **고주행군** |
| **3** | 5,049건 | 8,619km | 8km/일 | 0.6회/년 | 172.2일 | 초저주행군 |

**Cluster 2(고주행군)** 의 방문 주기가 57일로 가장 짧고 규칙적 → 예측이 가장 용이. **Cluster 3(초저주행군)** 은 0.6회/년으로 드문 방문 패턴을 보여 예측이 상대적으로 어렵다.
...
> **\[그림 3\]** Feature Set [A]→[C]로 갈수록 방문 이력 정보의 기여가 커진다. 특히 `gap_ma`가 추가된 [C]에서 성능이 크게 상승하며, 이는 **복잡한 모델 자체의 효과**라기보다 **과거 방문 이력 정보의 직접적인 예측력**이 크다는 점을 보여준다.
...
방문횟수가 증가할수록 Target 분산이 감소하고 R²가 향상된다. 이는 **더 많은 관측치가 차량별 평균 추정의 정확도를 높이기 때문**으로 해석할 수 있다. 다만 이 결과는 **Level 2 분석 표본이 3회 이상 방문 차량으로 제한되어 있다**는 전제 아래 읽어야 한다.
...
**실증 결과**: Level 2에서 앙상블 모델이 단일 모델보다 높은 성능을 보였다. 다만 이를 **Bias-Variance Tradeoff의 직접 증명**이라기보다, **비선형성·상호작용 학습 + 정규화**의 실용적 이점으로 해석하는 편이 더 적절하다.
...
**비즈니스 활용 관점:**
- 3회 이상 이력 고객(88.6%)에 한해 **±20.5일 오차** 수준의 평균 방문 주기 예측 가능
- Cluster 2(고주행군)는 57일 규칙적 주기 → 프로모션 타겟팅 후보
