...
EDA는 총 5개 차트로 구성되며, 각 차트가 제공하는 인사이트를 개별적으로 기술한다.
...
이러한 **분포의 이질성**은 곧 Level 1 (Per-Visit) 모델의 성능 한계(이론적 최대 R² ≈ 0.456)로 이어진다.
...
**핵심 인사이트 — 네 가지 차량 유형, 네 가지 방문 패턴**

| Cluster | 명칭 | 차량수 | 평균주행 | 평균간격 | 방문빈도 | 특성 |
|:-------:|------|:-----:|:--------:|:--------:|:--------:|------|
| 1 | **일반 승용** | 56,894대 | 76,570km | 115.0일 | 2.9회/년 | **가장 일반적인 패턴**, 중간 수준 |
| 0 | **저주행 일반** | 42,014대 | 51,231km | 171.3일 | 1.9회/년 | 주행 적고 방문 간 김 (시내 주차 위주 차량) |
| 2 | **영업용/업무용** | 16,016대 | 157,149km | **57.3일** | **6.7회/년** | 주행많고 방문 매우 잦음 (택시/택배 차량 추정) |
| 3 | **장기 미사용** | 5,049대 | 107,965km | 172.2일 | 2.0회/년 | 주행 대비 방문간격 김 (보관/시즌용 차량) |

**가장 중요한 발견**: Cluster 2(영업용/업무용)는 **전체의 12.8%**에 불과하지만, 평균 방문간격이 **57.3일**로 타 Cluster의 절반 이하다. 주행거리도 압도적으로 높아(157,149km) **차량 마모도가 높을수록 방문이 잦아지는 패턴**을 명확히 보여준다.

반면 Cluster 0(저주행 일반)과 3(장기 미사용)은 평균간격이 170일을 넘어 **1년에 2회 미만** 방문한다. 이들은 대부분의 예측 모델이 **정보 부족(Irreducible Error)** 으로 어려움을 겪을 구간이다.

이러한 Cluster별 이질성은 **모델링 전략**에 중요한 함의를 준다:
- **Cluster 2는 예측이 쉬운 집단** (규칙적인 방문 패턴)
- **Cluster 0/3은 예측이 어려운 집단** (드문 방문, 높은 변동성)
...
- **1회 방문**: 평균 171.4일 (이 첫방문만으로는 다음 방문 예측 난이도 높음)
- **3회 방문**: 126.9일
- **10회↑ 방문**: 80.2일
