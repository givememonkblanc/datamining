...
EDA는 총 5개 차트로 구성되며, 각 차트가 제공하는 인사이트를 개별적으로 기술한다. 다만 여기에는 `gap_ma`, `visit_freq_per_year` 등 **3장에서 정의되는 파생변수**도 포함되어 있다. 즉, 보고서의 전개 순서상 EDA가 먼저 나오지만, 실제 분석 파이프라인에서는 **전처리/파생변수 생성 후 EDA에 다시 활용**한 변수들이다.
